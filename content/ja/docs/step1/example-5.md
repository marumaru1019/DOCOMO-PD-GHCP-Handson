...
## 2. Copilot カスタマイゼーションとの違い

カスタムチャットモードと Copilot カスタマイゼーション（カスタム指示・プロンプトファイル）は、異なる目的で使用されます。

| 項目 | Copilot カスタマイゼーション | カスタムチャットモード |
|------|----------------------------|----------------------|
| **目的** | AIの**応答品質**を向上させる | **作業環境**を最適化する |
| **焦点** | 「どのように答えるか」 | 「どのツールで何をするか」 |
| **適用** | 自動的に全リクエストに適用 | 手動で切り替えて使用 |
| **設定内容** | コーディング規約、技術スタック | 使用ツール、作業モード |
...
1. **コマンドパレット** (`⇧⌘P`) → `Chat: New Mode File`
2. **保存場所を選択**:
   - **Workspace**: `.github/chatmodes/` フォルダ（チーム共有）
   - **User Profile**: 個人設定（複数ワークスペースで使用）
3. **モード名を入力** （チャットモードドロップダウンに表示される名前）
4. **ファイル内容を編集** （Front Matter + 指示内容）
